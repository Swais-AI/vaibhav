  ...
  const card = (
    <div
      className={`bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-3 transition-all ${
        href
          ? 'hover:border-orange-500/40 hover:bg-white/10 cursor-pointer'
          : ''
      }`}
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[11px] text-slate-300 font-semibold uppercase tracking-wide">
          {label}
        </p>

        <p className="text-xl font-black text-white mt-0.5 leading-tight">
          {value}
        </p>
    ...
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col">
      <div className="flex items-center justify-between mb-3">
        ...
        {action && (
          <Link
            href={action.href}
            className="text-[11px] text-orange-400 font-bold hover:underline shrink-0"
          >
            {action.label}
          </Link>
        )}
...
function AlertPill({
  ...
  return (
    <span
      className={`text-[10px] font-black px-2 py-0.5 rounded-md shrink-0 ${cls}`}
    >
      {p}
    </span>
  );
...
export default function ParentDashboard() {
  ...
  const generateParentInsight = async () => {
    try {
      ...
      // Use the dashboard data already fetched.
      if (!data) {
        throw new Error(
      ...
      if (
        ...
        insight =
          `Your child is showing strong academic progress, maintaining an overall average of ${averageScore.toFixed(
            2
          )} percent across recent assessments.\n\n` +
          `• Their strongest subject is ${strongestSubject}, showing good understanding and consistent performance.\n\n` +
          `• Their assignment completion is ${assignmentCompletion.toFixed(
            0
          )} percent, reflecting good responsibility and regular participation.\n\n` +
          `• Continue encouraging regular revision and short practice activities to maintain this positive progress.`;
      } else if (averageScore >= 50) {
        insight =
          `Your child is showing encouraging academic progress, maintaining an overall average of ${averageScore.toFixed(
            2
          )} percent across recent assessments.\n\n` +
        ...
        insight =
          `Your child may benefit from some additional academic support, based on the current overall average of ${averageScore.toFixed(
            2
          )} percent.\n\n` +
  ...
  // ── Extract real parent data ──────────────────────────────────────────
  //
  // Parent name is NOT hardcoded.
  //
  ...
  // data.parent_name
  //
  // If none exists, we show "Parent" rather than showing
  // another user's name.
  ...
  // Generate initials dynamically from the parent name.
  ...
  const pendingCount =
    data?.daily_summary?.assignments_pending ?? 0;

  const noticesCount =
    data?.daily_summary?.notices_today ?? 0;

  const alerts =
    (data?.alerts ?? []) as any[];

  const deadlines =
    (data?.upcoming_deadlines ?? []) as any[];

  const recs =
    (data?.smart_recommendations ?? []) as any[];

  const notifications =
    (data?.notifications ?? []) as any[];

  const subjectPerf =
    (data?.subject_performance ?? []) as any[];
  ...
  // ── Compute average quiz percentage ────────────────────────────────────
  ...
  const {
    displayed: dispAlertMsgs,
    translating: translatingAlerts,
  } = useTranslation(
    alertMsgTexts,
    language
  );
  ...
  const {
    displayed: dispDeadlineTitles,
    translating: translatingDeadlines,
  } = useTranslation(
    deadTitleTexts,
    language
  );
  ...
  const {
    displayed: dispRecMsgs,
    translating: translatingRecs,
  } = useTranslation(
    recMsgTexts,
    language
  );
  ...
  const assignmentCompletion =
    data?.assignment_completion_pct ?? null;

  const combinedLearning =
    assignmentCompletion !== null &&
    displayAvg !== null
      ? Math.round(
          assignmentCompletion * 0.6 +
            displayAvg * 0.4
        )
      : assignmentCompletion !== null
      ...
      <div className="flex-1 p-4 md:p-6 overflow-y-auto">
        <div className="max-w-6xl mx-auto space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="bg-red-500/10 text-red-400 p-6 rounded-xl text-center border border-red-500/20">
              {error}
            </div>
          ) : (
            <>
              ...
              {/* ── Row 3: Performance Summary + Recommendations ── */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <>
                      <div className="flex gap-2 mb-3 flex-wrap">
                        ...
                        <div className="flex-1 min-w-[80px] bg-green-50 rounded-xl p-2.5 text-center">
                          ...
                          <p className="text-xs font-black text-green-700 mt-0.5 truncate">
                            {strongest}
                          </p>
                        </div>
                        ...
                        <div className="flex-1 min-w-[80px] bg-red-50 rounded-xl p-2.5 text-center">
                          ...
                          <p className="text-xs font-black text-red-600 mt-0.5 truncate">
                            {weakest}
                          </p>
                        </div>
                ...
                <SectionCard title="💡 Recommendations">
                  {recs.length === 0 ? (
                    <EmptyState
                      icon="🌟"
                      text="Everything looks great! Keep it up."
                    />
                  ) : (
                    <div className="space-y-2">
